...
export const getLengths = async (chatId: number): Promise<number[]> => {
...
const insert = async (chatId: number, start: number): Promise<void> => {
  await pool.query(
...
export const isOpen = (chatId: number): Promise<boolean> => {
  return current.getId(chatId).then(Boolean);
};

export const open = async (chatId: number, start: number): Promise<void> => {
...
export const incrementLength = async (chatId: number, start: number): Promise<void> => {
  await pool.query(
    "UPDATE reflections SET length=length+1 WHERE user_id=$1 AND start_id=$2",
    [chatId, start]);
};

export const close = async (chatId: number, end: number, name: string): Promise<number> => {
...
export const cancel = async (chatId: number): Promise<void> => {
  const start = await current.getId(chatId);
